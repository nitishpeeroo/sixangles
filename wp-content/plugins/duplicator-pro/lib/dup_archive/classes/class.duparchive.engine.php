...
//                }
    ...
    private static function expandItems(&$expandState, $archiveHandle)
    {
        ...
        while ($moreToRead && (!$expandState->timedOut())) {
            ...
            if ($expandState->currentFileHeader != null) {
                ...
                if (self::passesFileExclusion($expandState->filteredDirectories, $expandState->filteredFiles, $expandState->currentFileHeader->relativePath)) {
                    try {
    ...
    private static function passesFileExclusion($directoryFilters, $fileFilters, $candidate)
    {
        $retVal = true;
        
        if (self::passesDirectoryExclusion($directoryFilters, $candidate)) {
