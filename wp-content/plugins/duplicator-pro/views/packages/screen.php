...
/* 
I have tried to put following function into following class, but it seems the filter didn't be triggered
*/
add_filter('set-screen-option', 'packages_set_option', 10, 3);
function packages_set_option($status, $option, $value) {
        if ( 'duplicator_pro_opts_per_page' == $option ) return $value;
        return $status;
}
...
class DUP_PRO_Package_Screen extends DUP_PRO_UI_Screen
{
	...
	public function __construct($page)
    {
	   ...
	   add_action( "load-".$page , array($this,'get_list_opts') );
		// add_filter('set-screen-option', array($this,'packages_set_option'), 10, 3); //this action didn't be triggered
    }
	...
	*/
	public function get_list_opts() 
	{
		...
		// add_filter('set-screen-option', array($this,'packages_set_option'), 10, 3);
	}
	
	// following function didn't be triggered	
	// public function packages_set_option($status, $option, $value) {
	//     if ( 'duplicator_pro_opts_per_page' == $option ) return $value;
	//     return $status;
	// }
}
