			...
			vc_map( array(
			    ...
			    'params' => array(
					array(
	                    ...
	                    'description' => __( 'Choose whether this is a top or bottom separator.', GAMBIT_ROW_SEPARATORS ) . '<br>' .
							__( 'Separators are meant to be full-width, please adjust your row settings to make it stretch to the whole width of the page.', GAMBIT_ROW_SEPARATORS ),
					),
					array(
						'type' => 'dropdown',
						'holder' => 'span',
					...
					array(
						...
						'param_name' => 'scale',
						'value' => '1',
	                    'description' => __( 'You can scale the separator to be larger or smaller. Use value between 0 and 1 to make the separator smaller, and more than 1 to make it larger.', GAMBIT_ROW_SEPARATORS ),
					),
					array(
						'type' => 'colorpicker',
					...
					array(
						'type' => 'textfield',
						'heading' => __( 'Decoration 1 Opacity', GAMBIT_ROW_SEPARATORS ),
						'param_name' => 'opacity1',
						'value' => '1',
	                    'description' => __( 'A value of 0-1. 0 means fully transparent, 1 means fully opaque. Put 0 here to remove the decor.', GAMBIT_ROW_SEPARATORS ),
					),
					array(
						'type' => 'textfield',
						'heading' => __( 'Decoration 2 Opacity', GAMBIT_ROW_SEPARATORS ),
						'param_name' => 'opacity2',
						'value' => '1',
	                    'description' => __( 'A value of 0-1. 0 means fully transparent, 1 means fully opaque. Put 0 here to remove the decor.', GAMBIT_ROW_SEPARATORS ),
					),
...
&lt;path class='gambit_sep_main' d='M-8,100c0,0,197-74,406-74s604,170,800,170s412-96,412-96v110H-8V100z'/></pre>",
		...
		public function render_shortcode( $atts, $content = null ) {
			...
			$svgClasses = 'gambit_separator';
			$svgClasses .= ' gambit_sep_' . $atts['location'];
			$svgClasses .= empty( $atts['flip'] ) ? '' : ' gambit_sep_flip';

			$svg = $svgs[ $atts['separator'] ]['svg'];

			if ( 'custom' == $atts['separator'] && ! empty( $atts['custom_svg'] ) ) {
			...
			$ret .= '<svg preserveAspectRatio="none" class="' . $svgClasses . '" viewBox="0 0 ' . $viewBoxWidth . ' ' . $viewBoxHeight . '" style="display: none; width: 100%; height: calc(' . $height . ' / ' . $svgs[ $atts['separator'] ]['width'] . ' * 100vw)" data-height="' . $height . '">' .
				$svg .
				'</svg>';
